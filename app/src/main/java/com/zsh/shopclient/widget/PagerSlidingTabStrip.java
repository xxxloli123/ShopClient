    ...
    private void notifyDataSetChanged() {
        ...
        for (int i = 0; i < tabCount; i++) {
            if (pager.getAdapter() instanceof IconTabProvider) {
                addIconTab(i, pager.getAdapter().getPageTitle(i), ((IconTabProvider) pager.getAdapter()).getPageIconResId(i));
            } else {
        ...
        getViewTreeObserver().addOnGlobalLayoutListener(new OnGlobalLayoutListener() {
            ...
            public void onGlobalLayout() {
                ...
                if (indicatorWrap) {
                    for (int i = 0; i < tabCount; i++) {
                        CharSequence s = pager.getAdapter().getPageTitle(i);
                        //计算所有tab的实际字体的宽
...
//        tab2.setCompoundDrawablePadding(1);//设置图片和text之间的间距
    ...
    private void addIconTab(final int position, CharSequence title, int resId) {
        TextView tab = new TextView(getContext());
        tab.setText(title);
        tab.setGravity(Gravity.CENTER);
        tab.setSingleLine();
