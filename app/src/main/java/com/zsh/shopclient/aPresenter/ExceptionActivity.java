...
import com.zsh.shopclient.adapter.ImageAdapter;
import com.zsh.shopclient.interfaceconfig.Config;
import com.zsh.shopclient.tool.FileHelper;
...
public class ExceptionActivity extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        ((TextView) findViewById(R.id.text_title)).setText(getString(getIntent().
                getIntExtra(getString(R.string.title), R.string.terraceFunctionException)));
        ...
        uris = new LinkedList<>();
        uris.add(Uri.parse("android.resource://" + getPackageName() + "/" + R.mipmap.icon_add));
...
//            反馈者电话};;pictures图片 参数：[opinionStr, pictures]
                ...
                StringBuilder jsonSb = new StringBuilder("{\"type\":\"" + getString(getIntent().
                        getIntExtra("txceptionType", R.string.functionException)));
                List<List<String>> lists = new TypeSqliteHelper(this,getString(R.string.sqlLibrary), null,1,null).query("user",new String[]{""},null);
                jsonSb.append("\",\"title\":\"").append(getString(getIntent().getIntExtra(getString(R.string.title),
                        R.string.terraceFunctionException))).
                        append("\",\"opinionContent\":\"").append(feedback.getText().toString().trim()).
                        append("\",\"userName\":\"").append(lists.get(0).get(0)).
                        append("\",\"userPhone\":\"").append(lists.get(0).get(0)).
                        append("\",\"userId\":\"").append(lists.get(0).get(0)).append("\"}");
